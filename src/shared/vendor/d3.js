d3 = function() {
  ...
  d3.geo.distance = function(a, b) {
    var UPPERDELTALAMBDA = (b[0] - a[0]) * d3_radians, PHI0 = a[1] * d3_radians, PHI1 = b[1] * d3_radians, sinUPPERDELTALAMBDA = Math.sin(UPPERDELTALAMBDA), cosUPPERDELTALAMBDA = Math.cos(UPPERDELTALAMBDA), sinPHI0 = Math.sin(PHI0), cosPHI0 = Math.cos(PHI0), sinPHI1 = Math.sin(PHI1), cosPHI1 = Math.cos(PHI1), t;
    return Math.atan2(Math.sqrt((t = cosPHI1 * sinUPPERDELTALAMBDA) * t + (t = cosPHI0 * sinPHI1 - sinPHI0 * cosPHI1 * cosUPPERDELTALAMBDA) * t), sinPHI0 * sinPHI1 + cosPHI0 * cosPHI1 * cosUPPERDELTALAMBDA);
  };
  ...
  function d3_geo_lengthLineStart() {
    ...
    function nextPoint(LAMBDA, PHI) {
      var sinPHI = Math.sin(PHI *= d3_radians), cosPHI = Math.cos(PHI), t = abs((LAMBDA *= d3_radians) - LAMBDA0), cosUPPERDELTALAMBDA = Math.cos(t);
      d3_geo_lengthSum += Math.atan2(Math.sqrt((t = cosPHI * Math.sin(t)) * t + (t = cosPHI0 * sinPHI - sinPHI0 * cosPHI * cosUPPERDELTALAMBDA) * t), sinPHI0 * sinPHI + cosPHI0 * cosPHI * cosUPPERDELTALAMBDA);
